...
span = 10;          % length of tx shaping filter in symbols
...
% Tptx = ((nzeros+nsympulse+span)*sps)/fs_tx;    % time per transmitted pulse (python script)
Tptx = ((nzeros+nsympulse)*sps)/fs_tx;    % time per transmitted pulse (gnuradio flowgraph)
...
save([path pulse_file_save_name],'yblock','fs_rx','fs_tx','sps', ...
...
subplot(5,3,4:6)
plot(abs(y1last)+6*shift_up); hold all
plot(abs(y2last)+3*shift_up)
plot(abs(y3last))
title('Zoomed In Last Pulse (Absolute Value)')
xlabel('Sample Number')
ylabel('Magnitude')
legend('Rx1','Rx2','Rx3')
axis tight
...
%Correlation Plots
[c12, lags12] = xcorr(y2last, y1last);
[c13, lags13] = xcorr(y3last, y1last);
cmag12 = abs(c12);
mean12 = mean(cmag12);
std12 = std(cmag12);
cmag13 = abs(c13);
...
subplot(5,3,12)
plot(lags12, abs(c12)/max(abs(c12)), 'b.-', 'MarkerSize', 12); hold all
plot(lags13, abs(c13)/max(abs(c13)), 'ro-', 'MarkerSize', 8)
axis([mlagidx12-3*peak_width mlagidx12+3*peak_width 0 1.1])
xlabel('Lag12')
ylabel('Correlation Magnitude')
title('Overlayed Normalized Correlations')
...
subplot(5,3,15)
plot(lags12, angle(c12)+3*pi, 'b-', 'MarkerSize', 12); hold all
plot(lags13, angle(c13), 'r-', 'MarkerSize', 8)
% axis([mlagidx12-3*peak_width mlagidx12+3*peak_width 0 1.1])
axis tight
xlabel('Lag12')
ylabel('Correlation Phase (Time)')
title('Overlayed Normalized Correlations')

%% Overlayed correlations and phases on separate figure
figure
subplot(3,1,1)
plot(lags12, abs(c12)/max(abs(c12)), 'b.-', 'MarkerSize', 12); hold all
plot(lags13, abs(c13)/max(abs(c13)), 'ro-', 'MarkerSize', 8)
axis([mlagidx12-3*peak_width mlagidx12+3*peak_width -0.1 1.2])
xlabel('Lag')
ylabel('Correlation Magnitude')
title('Overlayed Normalized Correlations')

subplot(3,1,2)
plot(lags12, angle(c12), 'b.-', 'MarkerSize', 12); hold all
plot(lags13, angle(c13), 'ro-', 'MarkerSize', 8)
axis([mlagidx12-3*peak_width mlagidx12+3*peak_width -3.1 3.1])
xlabel('Lag')
ylabel('Correlation Phase (Time)')
title('Overlayed Phase of Correlations')

subplot(3,1,3)
plot(lags12, angle(c12)-angle(c13), 'b.-', 'MarkerSize', 12); hold all
axis([mlagidx12-3*peak_width mlagidx12+3*peak_width -2*pi 2*pi])
xlabel('Lag')
ylabel('Correlation Phase Difference (Time)')
title('Overlayed Phase Difference of Correlations')
