function [coords, r_coord, loc_grid, reflect_grid, objective_vals, unique] = ...
    ms_music_dpd(rx_t, fs, fc, refPos, grid_defs, s_t, tx_pwr_dbm)
...
% create the matrix of test steering vectors
A = zeros(Nk*(M-1), Nlocgrid);
for loc_idx = 1:Nlocgrid % emitter grid index
    ...
    dtau = tau(2:end) - tau(1); % differential ToF between each rx and the first
    
%     for ii = 2:M
%         rangeDiffs(ii-1) = norm(p - refPos(:,ii)) - norm(p - refPos(:,1));
%     end
%     tdoas = rangeDiffs/c;
    a = [];
    for ii = 1:(M-1)
        a = [a; s_f.*exp(-1j*wk*dtau(ii))];
%         a = [a; exp(-1j*wk*dtau(ii))];
    end
    A(:,loc_idx) = a;
