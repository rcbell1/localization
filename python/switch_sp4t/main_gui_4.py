...
def ApplySwitching():
    ...
    dev[2].write(1,chr(v33)) #v22=1 switch port 1; v22=2 switch port 2
...
tk.Spinbox(root,values=(1,2,3,4),width=5,textvariable=V3).grid(row=3,column=4)
...
for configuration in dev[2]:
    for interface in configuration:
        ifnum = interface.bInterfaceNumber
        if not dev[2].is_kernel_driver_active(ifnum):
            continue
        try:
            dev[2].detach_kernel_driver(ifnum)
        except usb.core.USBError:
            pass
