...
loc_alg_params.type = ['sparse-dpd'];
...
Nrx = 6;
radius = 80;
origin = 0;     % receiver number to be assigned coordinate [0;0]
...
% targetPos1 = [10;-23];
% targetPos1 = [22;-2];
% targetPos1 = [54.683544303797476;22.278481012658233];   % exact grid point
% targetPos1 = [54.6835;22.2784];   % exact grid point
% targetPos1 = [54.683544303797476;26.3291];
% targetPos1 = [-18.2278; 6.0759];
% targetPos1 = [53.5;23.5];         % near grid point
% targetPos1 = [135;78.2785];          % outside convex hull
targetPos1 = [0.5;0.5];
% targetPos1 = [randi([-50 50]);randi([-50 50])];
% targetPos1 = [-2.02;-2.02];
% targetPos1 = [-11.0345;-11.0345];
% targetPos1 = refPos(:,1)+[20;21];
% targetPos1 = 50*[2*rand-1; 2*rand-1];
...
fig_bounds = ones(2,2*Nemitter);
fig_bounds(1,:) = -fig_bounds(1,:);
% temp2 = reshape(temp,4,[]).';
% r1 = 30;
% a = sqrt(2*radius(1)^2+4*radius(1)*cos(120*pi/180));
...
transmitter_params.tx_pwr_dbm = tx_pwr_dbm;
...
grid_xmin = grid_bounds(1) - adder + center(1);
grid_xmax = grid_bounds(2) + adder + center(1);
grid_ymin = grid_bounds(3) - adder + center(2);
grid_ymax = grid_bounds(4) + adder + center(2);
grid_numx = 30;
grid_numy = 30;
...
for ii = 1:Nemitter
    for jj = 1:num_jj  
        ...
        coords{ii,jj} = results.target{ii}.multi{jj}.coords;
        bias_coords{ii,jj} = results.target{ii}.multi{jj}.bias_coords;
        covar_coords{ii,jj} = results.target{ii}.multi{jj}.covar_coords;
        mse_coords(ii,jj) = results.target{ii}.multi{jj}.mse_coords;
        avg_snr_db{ii,jj} = results.target{ii}.multi{jj}.avg_snr_db;
        h{ii,jj} = results.target{ii}.multi{jj}.channel_taps;
        tx_raw{ii,jj} = results.target{ii}.multi{jj}.tx_raw;
        tx_up{ii,jj} = results.target{ii}.multi{jj}.tx_up;
        tx_delayed{ii,jj} = results.target{ii}.multi{jj}.tx_delayed;
        tx_multi{ii,jj} = results.target{ii}.multi{jj}.tx_multi;
        tx_noise{ii,jj} = results.target{ii}.multi{jj}.tx_noise;
%         tx_agc{ii,jj} = results.target{ii}.multi{jj}.tx_agc;
        multi_delays = results.target{ii}.multi{jj}.multi_delays;
        ranges = results.target{ii}.multi{jj}.ranges;
        tdoas_true(:,ii) = results.target{ii}.multi{jj}.tdoas_true;
        toas_true(:,ii) = results.target{ii}.multi{jj}.toas_true;
        ...
        switch loc_alg_params.type
            case {'dpd','direct-dpd','ms-dpd1','ms-dpd2','ms-music-dpd','ms-music-dpd2','sparse-dpd'}
                if contains(loc_alg_params.type, 'ms-dpd')
                    rcoords{ii,jj} = results.target{ii}.multi{jj}.reflection_coords;
                end
                dpd_grid{ii,jj} = results.target{ii}.multi{jj}.grid;
                if ~contains(loc_alg_params.type, 'sparse-dpd')
                    dpd_obj_vals{ii,jj} = results.target{ii}.multi{jj}.obj_vals;
                end
                if contains(loc_alg_params.type, 'sparse-dpd')
                    sparse_heatmap{ii,jj} = results.target{ii}.multi{jj}.sparse_heatmap;
                    sparse_max_coord{ii,jj} = results.target{ii}.multi{jj}.max_coord;
                    sparse_all_coords{ii,jj} = results.target{ii}.multi{jj}.all_coords;
                end
                
            case {'lsq', 'si', 'taylor'}
                tdoas_coarse{ii,jj} = results.target{ii}.multi{jj}.tdoas_coarse;
                tdoas_refined{ii,jj} = results.target{ii}.multi{jj}.tdoas_refined;
                prob_correlation(ii,jj) = results.target{ii}.multi{jj}.prob_correlation;
                prob_detection(ii,jj) = results.target{ii}.multi{jj}.prob_detection;
                corr_mag{ii,jj} = results.target{ii}.multi{jj}.corr_mag;
                corr_mag_bad{ii,jj} = results.target{ii}.multi{jj}.corr_mag_bad;
                
                if strcmp(loc_alg_params.type, 'lsq')
                    unique(ii,jj) = results.target{ii}.multi{jj}.unique;                    
                end
        end % switch
    end
end
