...
D = 3.5*Ts;   % max delay spread to test (s)
ND = 5;     % number of delay spreads to test
Ntaps = 10; % minimum number of channel taps to be used
sig = 0.02; % standard deviation of noise
...
y5 = zeros(ND, length(y1)+Ntaps);
delspreads = linspace(0, D, ND+1);
...
for ii = 1:ND
    ...
    [P, Q] = rat(fnew/fs);
    y2 = resample(y1, P, Q);
    h = exponential(Ntaps, 1/fnew, Di);
    hc{ii} = h;
    y3 = conv(y2,h);
    y4 = resample(y3, Q, P);
    y5(ii,1:length(y4)) = y4;
    
%     plot(1:Ntaps, abs(h), 'b.-')
%     xlabel('Tap Number')
%     ylabel('Magnitude')
%     title('Channel Tap Magnitude')
...
subplot(4,2,6)
plot(1:Ntaps, angle(h), 'r.-')
xlabel('Tap Number')
ylabel('Phase')
title('Channel Tap Phase')
axis([-inf inf -inf inf])

subplot(4,2,7:8)
faxis = fs*(-0.5:1/1024:0.5-1/1024);
x5_f = 10*log10(fftshift(1/1024*abs(fft(x5,1024)).^2));
y5_f = 20*log10(fftshift(1/1024*abs(fft(y5.',1024)).^2));
...
legend(lh(1:2), 'Transmitted','Received')
...
function [taps] = exponential(Ntaps, Ts, delay_spread)

f = @(x) exp(-x*Ts/delay_spread);
