function [outputArg1,outputArg2] = generate_delayed_signal(Nsym, fsym, ...
    sps_high, sps, span, beta, show_plots)

noise_bw = fsym*(beta+1);

% Create baseband signal at the receiver sample rate
x1 = 2*randi([0 1], Nsym, 1)-1;
x2 = upsample(x1,sps);
rrc = rcosdesign(beta, span, sps);
rrc = rrc/max(rrc);
emitter_sig = conv(rrc, x2);

% Add proper delays that correspond to target and emitter locations
[y1, true_delays, true_tdoas, ranges] = add_delay2(x, targetPos, refPos, ...
    fhigh, show_plots);
...
if show_plots == 1
    ...
    subplot(4,1,4)
    fs_g = fsym*sps_high/1e9; % GHz
    flen = length(rrc);
    w=kaiser(flen,8)';
    w=w/sum(w);
    fsym_g = fsym/1e9;
    Nfft = 2*2^nextpow2(length(w));
    faxis = sps_high*fsym_g*(-0.5:1/Nfft:0.5-1/Nfft);
